...
import {
  getGrammarVideoBySlug,
  getAllGrammarVideoSlugs,
  getRelatedGrammarVideos,
  getGrammarVideoVocabulary,
  getGrammarVideoQuizQuestions,
} from "@/lib/grammar-videos";
import { GrammarVideoContent } from "./grammar-video-content";
import { RelatedGrammarVideos } from "./related-videos";
...
export async function generateStaticParams() {
  const slugs = await getAllGrammarVideoSlugs();
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const video = await getGrammarVideoBySlug(slug);
  ...
  const description =
    video.description ||
    `Learn English grammar with "${video.title}". ${video.level ? `Level: ${video.level}.` : ""} Watch the video lesson with transcript, vocabulary, and quiz.`;
  ...
  return {
    title: `${video.title} - Grammar Video Lesson | Learne`,
    description,
    keywords: [
      `${video.title} video lesson`,
      `learn ${video.topic || "grammar"} English`,
      `${video.level || "English"} grammar video`,
      "English grammar tutorial",
      "grammar lesson online",
      "ESL grammar video",
    ...
    alternates: {
      canonical: `https://learne.org/grammar-videos/${slug}`,
    },
    twitter: {
      card: "summary_large_image",
      title: `${video.title} - Grammar Video Lesson`,
      description,
    ...
    openGraph: {
      ...
      description,
      type: "video.other",
      url: `https://learne.org/grammar-videos/${slug}`,
      siteName: "Learne",
      images: thumbnailUrl ? [thumbnailUrl] : undefined,
    },
  };
...
export default async function GrammarVideoPage({ params }: PageProps) {
  const { slug } = await params;
  const video = await getGrammarVideoBySlug(slug);
  ...
  const relatedVideos = await getRelatedGrammarVideos(slug, video.level, 6);
  const vocabulary = await getGrammarVideoVocabulary(video.id);
  const questions = await getGrammarVideoQuizQuestions(video.id);
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-secondary/30 flex flex-col">
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: video.title, url: `https://learne.org/grammar-videos/${slug}` },
        ]}
      />
      <Navbar />
      ...
      <main className="flex-1 container max-w-6xl mx-auto px-4 py-6">
        ...
        {relatedVideos.length > 0 && (
          <RelatedGrammarVideos videos={relatedVideos} currentLevel={video.level} />
        )}

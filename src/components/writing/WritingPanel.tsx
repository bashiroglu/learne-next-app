...
export function WritingPanel() {
  ...
  const wordCount = countWords(task1Text);
  const isWordCountMet = wordCount >= MIN_WORDS;
  const isTimeUp = task1Seconds === 0;

  const handleTextChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      // Start timer on first keystroke
      if (!task1TimerStarted && e.target.value.length > 0) {
        startTask1Timer();
      }
      setTask1Text(e.target.value);
    },
    [task1TimerStarted, startTask1Timer, setTask1Text]
  );
    ...
    <div className="panel-card h-full flex flex-col">
      ...
      <div className="panel-header flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Task 1 Response</span>
        </div>
      ...
      <div className="flex-1 p-4 overflow-hidden">
        <div className="editor-frame h-full relative">
          <textarea
            ...
            placeholder="Start typing your Task 1 response here. The timer will begin when you start writing..."
      ...
      <div className="panel-footer flex items-center justify-between">
        ...
        <div className="text-xs text-muted-foreground">
          {!task1TimerStarted && "Timer starts when you begin typing"}
          {task1TimerStarted && !isTimeUp && "Writing in progress..."}
          {isTimeUp && "Time is up!"}
        </div>
      </div>
    </div>
